...
static const qreal MenuItem_RightMarginForTextFontRatio = 2.0;
static const qreal MenuItem_RightMarginForArrowFontRatio = 1.0 / 4.0;
static const qreal MenuItem_VerticalMarginsFontRatio = 1.25 / 4.0;
...
static const qreal MenuItem_SeparatorHeightFontRatio = 1.0 / 8.0;
static const qreal MenuItem_CheckMarkVerticalInsetFontRatio = 1.0 / 5.0;
static const qreal MenuItem_IconRightSpaceFontRatio = 1.0 / 12.0;
...
static const bool MenuItemConsiderIcon = false;
...
void PhantomStyle::drawPrimitive(PrimitiveElement elem,
    ...
    case PE_IndicatorBranch: {
        ...
        // Branch
        {
            QPen pen = QPen(QColor(255, 255, 255, 22));

            if (option->state & State_Open)
            {

            }
            else if (option->state & State_Children)
            {

            }
            else if (option->state & State_Item)
            {
                painter->save();
    ...
    case PE_PanelButtonTool: {
        ...
        //@Pelly - this is where the toolbar hover rect is drawn, useful
        QRect r = option->rect;
        r.setHeight(r.height() - 1);

        Ph::PSave save(painter);
        Ph::paintSolidRoundRect(painter, r, rounding, swatch, fill);
...
void PhantomStyle::drawControl(ControlElement element,
    ...
    case CE_ToolBar: {
        ...
        else
        {
            ...
            painter->setPen(swatch.color(S_window_divider));
            painter->drawLine(
                QPoint { option->rect.bottomLeft().x(), option->rect.bottomLeft().y() + 0 },
                QPoint { option->rect.bottomRight().x(), option->rect.bottomRight().y() + 0 });
    ...
    case CE_MenuItem: {
        ...
        const bool hasIcon = (Phantom::MenuItemConsiderIcon) ? !menuItem->icon.isNull() : true;
...
int PhantomStyle::pixelMetric(PixelMetric metric, const QStyleOption* option,
    ...
    case PM_TreeViewIndentation: {
        if (widget)
            return widget->fontMetrics().height() * 1.25f;
        val = 12;
        break;
    }
    default:
...
QSize PhantomStyle::sizeFromContents(ContentsType type,
    ...
    case CT_MenuItem: {
        ...
        if (!menuItem->icon.isNull() || !Phantom::MenuItemConsiderIcon) {
