...
namespace Soulcast
{
	...
	public:
		...
		static void DrawSprite(BitmapRegion* sprite, int32 x, int32 y);
		static void DrawSpriteRegion(BitmapRegion* sprite, int32 x, int32 y, int32 sprX, int32 sprY, int32 sprWidth, int32 sprHeight, SpriteFlip flip = FLIP_NONE);

		static void ApplyMosaicEffect(int32 size);

	public:
		static void InitScreenInfo(ScreenInfo* screen, int32 width, int32 height, uint16* frameBuffer = nullptr);
	};
}